							...
							<table class="table table-hover table-normal">
								<thead>
									<tr>
										...
										<th>Cantidad</th>
										<th>Estado</th>
										@if(auth()->user()->can('products.edit') || auth()->user()->can('products.active') || auth()->user()->can('products.deactive') || auth()->user()->can('products.delete'))
										<th>Acciones</th>
										@endif
									</tr>
								</thead>
								<tbody>
									...
									<tr>
										...
										<td>
											@if($product->min>=$product->qty)
											<span class="text-danger">{{ $product->qty }}</span>
											@elseif($product->max<=$product->qty && $product->max>0)
